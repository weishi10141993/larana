...
bool cosmic::BeamFlashTrackMatchTaggerAlg::InDriftWindow(double start_x, double end_x, geo::Geometry const& geom){
  // Allow a tiny tolerance for fit movement
  if(start_x+0.5 < 0. || end_x+0.5 < 0.) return false;
  if(start_x-0.5 > 2*geom.DetHalfWidth() || end_x-0.5 > 2*geom.DetHalfWidth()) return false;
  return true;
}
