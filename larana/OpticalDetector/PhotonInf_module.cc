...
namespace opdet 
{
    ...
    , LogFileName{pset.get<std::string>("LogFileName")}
    {
        ...
        if(FillTree)
        {
            ...
            {
                PhInf->Branch(Form("Channel_%03d_det", channel),  &(num_detph[channel]), Form("Channel_%03d_det/I", channel));
                PhInf->Branch(Form("Channel_%03d_ref", channel),  &(num_refph[channel]), Form("Channel_%03d_ref/I", channel));
                PhInf->Branch(Form("Channel_%03d_tot", channel),  &(num_totph[channel]), Form("Channel_%03d_tot/I", channel));
            }
        }
    }
