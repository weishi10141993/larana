...
BOOST_AUTO_TEST_CASE(FillAccumulator_checkBelowThreshold)
{
  ...
  opdet::FillAccumulator(
    AccumIndex, HitIndex, PE, FlashThreshold, Binned, Contributors, FlashesInAccumulator);
...
BOOST_AUTO_TEST_CASE(FillAccumulator_checkMultipleHits)
{
  ...
  opdet::FillAccumulator(
    AccumIndex, HitIndex2, PE, FlashThreshold, Binned, Contributors, FlashesInAccumulator);
...
BOOST_AUTO_TEST_CASE(FillHitsThisFlash_EmptyContributors)
{
  ...
  opdet::FillHitsThisFlash(Contributors, Bin, HitClaimedByFlash, HitsThisFlash);
...
BOOST_AUTO_TEST_CASE(FillHitsThisFlash_NoPrevClaimedHits)
{
  ...
  std::vector<std::vector<int>> Contributors(vector_size);
  int Bin = 0;
  Contributors[0].push_back(1);
  Contributors[0].push_back(3);
...
BOOST_AUTO_TEST_CASE(FillHitsThisFlash_MultipleContributorVectors)
{
  ...
  Contributors[0].push_back(1);
  Contributors[0].push_back(3);
  Contributors[1].push_back(5);
  Contributors[1].push_back(6);
  ...
  opdet::FillHitsThisFlash(Contributors, 1, HitClaimedByFlash, HitsThisFlash);
...
BOOST_AUTO_TEST_CASE(ClaimHits_NoHitsThisFlash)
{
  ...
  opdet::ClaimHits(HitVector, HitsThisFlash, FlashThreshold, HitsPerFlash, HitClaimedByFlash);
...
BOOST_AUTO_TEST_CASE(ClaimHits_TwoHitsThisFlash_BelowThreshold)
{
  ...
  std::vector<int> HitsThisFlash;
  HitsThisFlash.push_back(0);
  HitsThisFlash.push_back(1);
  std::vector<std::vector<int>> HitsPerFlash;
...
BOOST_AUTO_TEST_CASE(AddHitContribution_AddFirstHit)
{
  ...
  opdet::AddHitContribution(
    currentHit, MaxTime, MinTime, AveTime, FastToTotal, AveAbsTime, TotalPE, PEs);
...
BOOST_AUTO_TEST_CASE(GetLikelihoodLateLight_BackwardsTime)
{
  ...
  double result = opdet::GetLikelihoodLateLight(iPE, iTime, iWidth, jPE, jTime, jWidth);
...
BOOST_AUTO_TEST_CASE(MarkFlashesForRemoval_NoFlashes)
{
  ...
  opdet::MarkFlashesForRemoval(FlashVector, BeginFlash, MarkedForRemoval);
...
BOOST_AUTO_TEST_CASE(RemoveFlashesFromVectors_IgnoreFirstFlash)
{
  ...
  opdet::RemoveFlashesFromVectors(MarkedForRemoval, FlashVector, BeginFlash, RefinedHitsPerFlash);
